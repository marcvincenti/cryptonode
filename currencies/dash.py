...
def masternodes(event, context):
	...
	block_number = re.findall(pattern, content[23]).pop().replace(" ", "")
	block_difficulty = re.findall(pattern, content[24]).pop().replace(" ", "")
	masternodes_count = re.findall(pattern, content[26]).pop().replace(" ", "")
